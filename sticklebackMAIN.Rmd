...
library(DHARMa) # 0.4.5
library(dplyr) # 1.0.9 
library(fasterize) # 1.0.3
library(ggnewscale) # 0.4.7
library(ggplot2) # 3.3.6
library(ggpubr) # 1.1.3 
library(glmmTMB) # 1.1.3
library(grid) # 4.2.1
library(gstat) # 2.0-9
library(Hmisc) # 4.7-0 
library(INLA) # 22.05.07  
library(lubridate) # 1.8.0 
library(mctest) # 1.3.1
library(MetBrewer) # 0.2.0 
library(MuMIn) # 1.46.0
library(ncf) # 1.3-2
library(nlme) # 3.1-157
library(RANN) # 2.6.1
library(raster) # 3.5-21
library(RColorBrewer) # 1.1-3
library(rgdal) # 1.5-32
library(sdmTMB) # 0.1.0
...
The juvenile dataset includes 1700+ data points from detonations. Here, the data are cleaned up and some potential drivers added (wave exposure, distance to baseline, year, cormorants, seals, fishing pressure).
...
## create a subset spatio-temporal subset ##
df_sub = df_juv[
...
p1 = ggplot(data = df_sub, aes(conn35)) +
  ...
  theme_bw() +
  labs(x = "Habitat availability", y = "Count", title = "Limit = 3.5") +
...
p2 = ggplot(data = df_sub, aes(conn32)) +
  geom_histogram(binwidth = 1000) +
  theme_bw() +
  labs(x = "Habitat availability", y = "Count", title = "Limit = 3.2") +
  lims(x = c(0, maxV), y = c(0, 510)) +
  theme_sets 
